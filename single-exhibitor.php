...
/**
 * The Template for displaying all exhibitors post.
 *
 * @package uncode
 */
...
while (have_posts()):
	...
	function show_taxes($tax){
		if($tax == 'email'){
			return '<div style="text-transform: lowercase; color: #222222">' . strip_tags(get_the_term_list( $post->ID, $tax)) .'</div><br>';
		}else if($tax == 'strona'){
			return '<div style="text-transform: lowercase;"><a style="color: #222222; text-decoration: underline;" href="'. strip_tags(get_the_term_list( $post->ID, $tax)) .'">' . strip_tags(get_the_term_list( $post->ID, $tax)) .'</a></div><br>';
		}		
		else{
			return '<div style="color: #222222">' . strip_tags(get_the_term_list( $post->ID, $tax)) .'</div><br>';
		}
	}
	...
	if ($show_title) {
		...
		$title_content .= '<center><img src="'. get_the_post_thumbnail_url($post->ID, 'full') .'" alt="logo-exhibitor" width=300></center>';
		$title_content .= '<div class="post-info">' . 'Hala: '. show_taxes('hall') . 'Numer Stoiska: ' .show_taxes('stoisko') . 'Adres email: '. show_taxes('email') . 'Numer telefonu: '. show_taxes('telefon') . 'Strona Internetowa: ' .show_taxes('strona') .'</div>';
		$title_content .= '</div>';
		$title_content .= apply_filters( 'uncode_after_body_title', '' );
	}

	/** Build content **/
	$the_content = '<strong style="color: #222222 !important">Opis Wystawcy</strong><br>';
